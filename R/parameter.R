...
)
{
  ...
  kwb.utils::catAndRun(dbg = dbg, sprintf("Writing parameters to '%s'", file), {
    writeLines(
      parameterLines(parameters, sep = sep, digits_exp = digits_exp), 
      con = file
    )
  })
}
...
#'   provided by \code{kwbGompitz:::readParameters}
#' @param sep column separator. Default: ";"
#' @param digits_exp number of exponent digits in scientific notation, such as
#'   "1.23e+01" (digits_exp = 2L, the default) or "1.23e+001" (digits_exp = 3L)
...
#'
parameterLines <- function(parameters, sep = ";", digits_exp = 2L)
{
  c(
    parameterLines_header(parameters, sep),
    parameterLines_category(parameters, sep), # may be NULL
    parameterLines_stratum(parameters, sep, digits_exp)
  )
}
...
parameterLines_stratum <- function(parameters, sep, digits_exp = 2L)
{
  ...
  result <- vapply(
    ...
    FUN = function(stratum) {
      ...
      # Get the estimates
      estimates <- get(stratum_info, "estimates")
      
      # Format the estimates
      estimates <- sprintf("%0.6e", estimates)
      
      # Reformat the scientific strings to the given number of exponent digits
      estimates <- reformatScientific(estimates, digits_exp)

      # Set the estimates in stratum_info
      stratum_info$estimates <- estimates
...
# reformatScientific -----------------------------------------------------------
reformatScientific <- function(x, digits_exp)
{
  # Regular expression matching the scientific number string
  pattern <- "^(.*)([+-]\\d+)$"
  
  # Alter the exponent to the given number of digits
  exponents <- gsub(pattern, "\\2", x, perl = TRUE)
  
  # Compose format specifier
  fmt <- paste0("%+0", digits_exp + 1L, "d")
  
  # Set new exponent strings
  paste0(gsub(pattern, "\\1", x), sprintf(fmt, as.integer(exponents)))
}
